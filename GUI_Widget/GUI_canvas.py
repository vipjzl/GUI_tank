...
class Application(Frame):
    ...
    def createWidget(self):
        ...
        # 画一个矩形.
        rect = self.canvas.create_rectangle(50, 50, 150, 100)
        print(type(rect))
        # 画一个椭圆.坐标两双。为椭圆的边界矩形左上角和底部右下角
        oval = self.canvas.create_oval(50, 50, 150, 100)
        global photo
        photo = PhotoImage(file="../images/017.GIF")
        self.canvas.create_image(150, 170, image=photo)
        Button(self, text="画 10 个矩形", command=self.draw50Recg).pack(side="left")
        Button(self, text="清空", command=self.canvas).pack(side="left")
